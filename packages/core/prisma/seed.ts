...
const categories = ['Транспорт','Недвижимость','Электроника','Дом и сад','Одежда и обувь','Детские товары','Работа','Услуги','Животные','Отдам бесплатно','Обмен','Другое'];
...
async function main() {
  ...
  for (const [index, name] of categories.entries()) await db.category.upsert({
    ...
    create: { communityId: community.id, name, slug: slug(name, index), sortOrder: index },
  });
  for (const rawId of (process.env.INITIAL_ADMIN_TELEGRAM_IDS || '').split(',').map(v => v.trim()).filter(Boolean)) {
    const user = await db.user.upsert({ where: { telegramUserId: BigInt(rawId) }, update: {}, create: { telegramUserId: BigInt(rawId), firstName: 'Owner' } });
