...
function Header({ botUsername }: { botUsername?: string }) {
  ...
  return <header className="public-header"><a className="public-brand" href="/"><span>CB</span><b>Community Board</b></a><nav><a href="/pricing">Тарифы</a><a href="/docs">Как работает</a><a href="/support">Поддержка</a></nav><a className="public-login" href={signedIn ? "/owner" : "/login"} onClick={() => void track(signedIn ? "dashboard_return" : "web_login_open")}>{signedIn ? "Открыть кабинет" : "Войти через Telegram"}</a></header>;
}
