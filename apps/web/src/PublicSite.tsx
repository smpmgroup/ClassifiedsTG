...
function Header({ botUsername }: { botUsername?: string }) {
  const [signedIn, setSignedIn] = useState(Boolean(sessionStorage.getItem("platformToken")));
  useEffect(() => {
    fetch("/api/platform/me")
      .then((response) => setSignedIn(response.ok))
      .catch(() => undefined);
  }, []);
  return <header className="public-header"><a className="public-brand" href="/"><span>AD</span><b>Adnecta</b></a><nav><a href="/pricing">Тарифы</a><a href="/docs">Как работает</a><a href="/support">Поддержка</a></nav><a className="public-login" href={signedIn ? "/owner" : "/login"} onClick={() => void track(signedIn ? "dashboard_return" : "web_login_open")}>{signedIn ? "Открыть кабинет" : "Войти через Telegram"}</a></header>;
}

function Footer() {
  return <footer className="public-footer"><div><b>Adnecta</b><span>Marketplace for every community.</span></div><nav><a href="/terms">Условия</a><a href="/privacy">Конфиденциальность</a><a href="/prohibited">Запрещённые товары</a><a href="/support">Поддержка</a></nav><small>Закрытая beta · все цифровые услуги и подписки оплачиваются только Telegram Stars <a className="service-owner-entry" href="/platform-login" aria-label="Вход владельца платформы">·</a></small></footer>;
}
...
function Landing({ data }: { data: SiteData }) {
  return <>
    ...
    <section className="public-roles"><small>ПОНЯТНЫЕ РОЛИ</small><h2>Каждому — свой рабочий интерфейс</h2><div><article><b>Владелец сообщества</b><p>Подключает группы, выбирает модель монетизации, видит Stars, выплаты и состояние доски в большом веб-кабинете.</p></article><article><b>Администратор</b><p>Проверяет объявления, управляет правилами, категориями и участниками прямо внутри Telegram.</p></article><article><b>Участник</b><p>Ищет, публикует, сохраняет и управляет объявлениями без отдельной регистрации и лишних приложений.</p></article></div></section>
    ...
    <section className="public-faq"><small>ЧАСТЫЕ ВОПРОСЫ</small><h2>До запуска всё прозрачно</h2><details><summary>Нужен собственный бот?</summary><p>Нет. Один бот платформы безопасно обслуживает независимые сообщества и открывает правильную доску по ссылке группы.</p></details><details><summary>Кто может публиковать бесплатно?</summary><p>Владелец задаёт порог активности и может вручную предоставить бесплатный доступ. Остальные оплачивают публикацию Stars.</p></details><details><summary>Где происходит модерация?</summary><p>Администраторы получают очередь и уведомления внутри Telegram, а коммерческие настройки остаются в веб-кабинете владельца.</p></details><details><summary>Можно ли отключить доску?</summary><p>Да. Владелец управляет подключением, может приостановить работу и запросить удаление данных с предусмотренным периодом защиты.</p></details></section>
