...
function expectStatus(
  ...
  if (!accepted.includes(actual))
    throw new Error(`${name}: expected ${accepted.join("/")}, got ${actual}`);
  checks.push(name);
}
...
async function cleanup() {
  ...
  if (webLoginIntentIds.length)
    await prisma.webLoginIntent.deleteMany({
      where: { id: { in: webLoginIntentIds } },
    });
  if (userIds.length)
...
try {
  ...
  const [orgA, orgB] = await Promise.all([
    prisma.organization.create({
      data: {
        name: `Beta A ${stamp}`,
        slug: `beta-a-${stamp}`,
        members: { create: { userId: moderator.id, role: "owner" } },
      },
    }),
    prisma.organization.create({
      data: {
  ...
  const [communityA, communityB] = await Promise.all([
    prisma.community.create({
      data: {
        telegramChatId: -(telegramBase + 100n),
        name: `Beta community A ${stamp}`,
        slug: `beta-community-a-${stamp}`,
        inviteUrl: "https://t.me/beta_a",
        organizationId: orgA.id,
        minMonthlyMessagesForFree: 2,
      },
    }),
    prisma.community.create({
      data: {
        telegramChatId: -(telegramBase + 200n),
        name: `Beta community B ${stamp}`,
        slug: `beta-community-b-${stamp}`,
        inviteUrl: "https://t.me/beta_b",
  ...
  const listingData = {
    title: `Beta listing ${stamp}`,
    description: "Complete beta acceptance listing",
    price: 25,
    locationText: "Tarragona",
  };
  ...
  await Promise.all([
    fs.writeFile(
      path.join(process.env.UPLOAD_DIR || "/app/uploads", fileA),
      Buffer.from("beta-a"),
    ),
    fs.writeFile(
      path.join(process.env.UPLOAD_DIR || "/app/uploads", fileB),
      Buffer.from("beta-b"),
    ),
  ]);
  ...
  const memberA = token({
    ...
    communityId: communityA.id,
    role: "member",
    telegramUserId: String(shared.telegramUserId),
  });
  const memberB = token({
    userId: shared.id,
    communityId: communityB.id,
  ...
  const moderatorA = token({
    userId: moderator.id,
    communityId: communityA.id,
    role: "owner",
    telegramUserId: String(moderator.telegramUserId),
  });
  const platformA = token({
    scope: "platform",
    userId: moderator.id,
    telegramUserId: String(moderator.telegramUserId),
    platformRole: "user",
  });
  const platformStaffUnverified = token({
    scope: "platform",
    userId: platformStaff.id,
    telegramUserId: String(platformStaff.telegramUserId),
    platformRole: "platform_admin",
    mfa: false,
  });
  ...
  expectStatus(
    "current Telegram Bearer account overrides a stale privileged browser cookie",
    isolatedAccount.status,
    200,
  );
  if (
    isolatedAccount.body.user.id !== moderator.id ||
    isolatedAccount.body.user.platformRole !== "user"
  )
    throw new Error(
  ...
  const unpaidFreeMode = await request(
    `/api/platform/communities/${communityA.id}/monetization`,
    platformA,
    {
      method: "PATCH",
  ...
  expectStatus(
    "free-for-all requires owner subscription",
    unpaidFreeMode.status,
    402,
  );
  ...
  expectStatus(
    "subscribed owner enables free-for-all",
    (
      await request(
        `/api/platform/communities/${communityA.id}/monetization`,
        platformA,
  ...
  checks.push(
    "owner Stars models enforce paid subscription for a fully free board",
  );
  expectStatus(
    "admin grants manual free publication",
    (
      await request(`/api/admin/users/${seller.id}`, moderatorA, {
        method: "PATCH",
  ...
  const sellerOverride = await prisma.communityMember.findUniqueOrThrow({
    where: {
      communityId_userId: { communityId: communityA.id, userId: seller.id },
    },
  });
  if (!sellerOverride.freePublicationOverride)
    throw new Error("manual free publication was not stored");
  checks.push("administrator can maintain the manual free-publication list");

  const webStart = await request("/api/auth/platform/web/start", "", {
    method: "POST",
    body: "{}",
  });
  ...
  if (
    ...
    !String(webStart.body.botUrl).includes(
      `?start=login_${webStart.body.token}`,
    ) ||
    !String(webStart.body.telegramAppUrl).includes(
      `&start=login_${webStart.body.token}`,
    ) ||
    !String(webStart.body.botUrl).startsWith(
      `https://t.me/${webStart.body.botUsername}?`,
    )
  )
  ...
  expectStatus(
    "platform owner starts dedicated browser login",
    platformOwnerStart.status,
    200,
  );
  if (
    !String(platformOwnerStart.body.botUrl).includes(
      `?start=loginp_${platformOwnerStart.body.token}`,
    )
  )
    throw new Error("platform owner login lost its destination");
  const platformOwnerTokenHash = crypto
    .createHash("sha256")
    .update(platformOwnerStart.body.token)
    .digest("hex");
  const platformOwnerIntent = await prisma.webLoginIntent.findUniqueOrThrow({
    where: { tokenHash: platformOwnerTokenHash },
  });
  webLoginIntentIds.push(platformOwnerIntent.id);
  checks.push(
    "platform owner Telegram handoff preserves the browser destination",
  );
  const webTokenHash = crypto
    .createHash("sha256")
    .update(webStart.body.token)
    .digest("hex");
  const webIntent = await prisma.webLoginIntent.findUniqueOrThrow({
    where: { tokenHash: webTokenHash },
  });
  ...
  if (claimedWebLogin.count !== 1)
    throw new Error("web login could not be claimed");
  ...
  if (duplicateClaim.count !== 0)
    throw new Error("web login could be claimed by a second user");
  checks.push("Telegram confirmation is atomically single-claim");
  const webComplete = await request("/api/auth/platform/web/status", "", {
    method: "POST",
    body: JSON.stringify({ token: webStart.body.token }),
  });
  expectStatus(
    "website exchanges Telegram confirmation",
    webComplete.status,
    200,
  );
  if (webComplete.body.status !== "complete" || !webComplete.body.accessToken)
    throw new Error("website login did not issue a platform session");
  expectStatus(
    "website session opens owner dashboard",
    (await request("/api/platform/me", webComplete.body.accessToken)).status,
    200,
  );
  const webRetry = await request("/api/auth/platform/web/status", "", {
    method: "POST",
    body: JSON.stringify({ token: webStart.body.token }),
  });
  expectStatus(
    "website exchange tolerates response retry",
    webRetry.status,
    200,
  );
  if (webRetry.body.accessToken !== webComplete.body.accessToken)
    throw new Error("web login retry was not idempotent");
  ...
  if (
    !own.body.some((item: any) => item.id === draftA.id) ||
    own.body.some((item: any) => item.id === draftB.id)
  )
    throw new Error("own listings crossed tenant boundary");
  ...
  if (
    favorites.body.length !== 1 ||
    favorites.body[0].listing.id !== publishedA.id
  )
    throw new Error("favorites crossed tenant boundary");
  ...
  if (
    ...
    !protectedUrl.startsWith(`/api/media/${imageA.id}?token=`) ||
    "storageKey" in detail.body.images[0]
  )
    throw new Error("listing image did not receive a sanitized signed URL");
  expectStatus(
    "signed image delivery",
    (await request(protectedUrl, memberA)).status,
    200,
  );
  ...
  expectStatus(
    "legacy upload path denied",
    (await fetch(`${baseUrl}/uploads/${fileA}`)).status,
    404,
  );
  ...
  expectStatus(
    ...
    (
      await request(`/api/listings/${draftB.id}`, memberA, {
        method: "PATCH",
  ...
  expectStatus(
    ...
    (
      await request(`/api/listings/${draftB.id}/transition`, memberA, {
        method: "POST",
  ...
  expectStatus(
    "cross-tenant payment denied",
    (
      await request(`/api/listings/${draftB.id}/payment-link`, memberA, {
        method: "POST",
      })
    ).status,
    404,
  ...
  expectStatus(
    ...
    (
      await request(`/api/listings/${draftB.id}/images`, memberA, {
        method: "POST",
        body: new FormData(),
      })
    ).status,
  ...
  expectStatus(
    "cross-tenant image delete denied",
    (
      await request(`/api/listings/${draftB.id}/images/${imageB.id}`, memberA, {
        method: "DELETE",
      })
    ).status,
    404,
  ...
  if (unchanged.title === "MUTATED")
    throw new Error("cross-tenant listing was mutated");
  checks.push("cross-tenant mutation left data unchanged");

  expectStatus(
    "member cannot open admin dashboard",
    (await request("/api/admin/dashboard", memberA)).status,
    403,
  );
  ...
  if (moderationBefore.body.some((item: any) => item.id === pendingB.id))
    throw new Error("moderation queue crossed tenant boundary");
  ...
  expectStatus(
    "web community administrator reaches own dashboard",
    webAdminOwn.status,
    200,
  );
  expectStatus(
  ...
  expectStatus(
    "free active member submits listing",
    (
      await request(`/api/listings/${draftA.id}/transition`, memberA, {
        method: "POST",
        body: JSON.stringify({ status: "pending" }),
      })
    ).status,
  ...
  expectStatus(
    "tenant moderator publishes listing",
    (
      await request(`/api/admin/listings/${draftA.id}/transition`, moderatorA, {
        method: "POST",
  ...
  if (
    !board.body.some((item: any) => item.id === draftA.id) ||
    board.body.some((item: any) => item.id === publishedB.id)
  )
    throw new Error("published board crossed tenant boundary");
  checks.push("publish and board flow remains tenant isolated");

  expectStatus(
    "organization finance denied across tenant",
    (await request(`/api/platform/organizations/${orgB.id}/finance`, platformA))
      .status,
    403,
  );
  expectStatus(
    "ordinary platform user denied owner console",
    (await request("/api/platform/admin/reliability", platformA)).status,
    403,
  );
  ...
  expectStatus(
    "Telegram-authenticated platform session reaches owner console without mandatory 2FA",
    (await request("/api/platform/admin/reliability", platformStaffUnverified))
      .status,
    200,
  );
  expectStatus(
    "platform admin without tenant support assignment stays isolated",
    (
      await request(
        `/api/platform/support/${supportTicket.id}/messages`,
        platformStaffUnverified,
        {
          method: "POST",
          body: JSON.stringify({
  ...
  const twoFactorSetup = await request(
    "/api/platform/security/two-factor/setup",
    platformStaffUnverified,
    { method: "POST", body: "{}" },
  );
  expectStatus(
    "privileged user starts encrypted 2FA setup",
    twoFactorSetup.status,
    200,
  );
  const setupCode = totpAt(twoFactorSetup.body.secret);
  const twoFactorConfirm = await request(
  ...
  if (
    !Array.isArray(twoFactorConfirm.body.backupCodes) ||
    twoFactorConfirm.body.backupCodes.length !== 10
  )
    throw new Error("2FA backup codes were not issued exactly once");
  checks.push("2FA issued ten one-time backup codes");
  expectStatus(
    "MFA-verified platform session reaches owner console",
    (
      await request(
        "/api/platform/admin/reliability",
        twoFactorConfirm.body.accessToken,
      )
    ).status,
    200,
  );
  expectStatus(
    "MFA-verified support bypass succeeds",
    (
      await request(
        `/api/platform/support/${supportTicket.id}/messages`,
        twoFactorConfirm.body.accessToken,
        {
          method: "POST",
          body: JSON.stringify({
            message: "verified internal note",
            internal: true,
          }),
        },
      )
  ...
  const staffWebStart = await request("/api/auth/platform/web/start", "", {
    method: "POST",
    body: "{}",
  });
  ...
  const staffWebIntent = await prisma.webLoginIntent.findUniqueOrThrow({
    where: {
      tokenHash: crypto
        .createHash("sha256")
        .update(staffWebStart.body.token)
        .digest("hex"),
    },
  });
  webLoginIntentIds.push(staffWebIntent.id);
  await prisma.webLoginIntent.update({
    where: { id: staffWebIntent.id },
    data: {
      status: "claimed",
      userId: platformStaff.id,
      claimedAt: new Date(),
  ...
  const staffWebStatus = await request("/api/auth/platform/web/status", "", {
    method: "POST",
    body: JSON.stringify({ token: staffWebStart.body.token }),
  });
  expectStatus(
    "privileged website login completes after Telegram confirmation",
    staffWebStatus.status,
    200,
  );
  if (!staffWebStatus.headers.get("set-cookie")?.includes("platform_session="))
    throw new Error(
      "completed web login did not create a browser-wide session",
    );
  checks.push("completed web login creates a browser-wide cookie session");
  if (
    staffWebStatus.body.status !== "complete" ||
    !staffWebStatus.body.accessToken
  )
  ...
  expectStatus(
    "used TOTP step cannot be replayed",
    (
      await request("/api/auth/platform/two-factor", platformStaffUnverified, {
        method: "POST",
        body: JSON.stringify({ challengeToken: challenge, code: setupCode }),
      })
    ).status,
    401,
  );
  const backupChallenge = token({
    scope: "platform_2fa",
    userId: platformStaff.id,
    nonce: `beta-backup-${stamp}`,
  });
  const backupLogin = await request(
    "/api/auth/platform/two-factor",
    platformStaffUnverified,
    {
      method: "POST",
      body: JSON.stringify({
        challengeToken: backupChallenge,
        code: twoFactorConfirm.body.backupCodes[0],
      }),
    },
  );
  expectStatus(
    "backup code completes privileged login",
    backupLogin.status,
    200,
  );
  expectStatus(
    "2FA challenge cannot be replayed",
    (
      await request("/api/auth/platform/two-factor", platformStaffUnverified, {
        method: "POST",
        body: JSON.stringify({
  ...
  await prisma.community.update({
    where: { id: communityB.id },
    data: { tenantStatus: "suspended" },
  });
  expectStatus(
    "suspended tenant denied",
    (await request("/api/me", memberB)).status,
    403,
  );
  expectStatus(
    "unrelated tenant survives suspension",
    (await request("/api/me", memberA)).status,
    200,
  );
  await prisma.community.update({
    where: { id: communityB.id },
  ...
  await prisma.communityMember.update({
    where: {
      communityId_userId: { communityId: communityB.id, userId: shared.id },
    },
    data: {
      enforcementStatus: "banned",
      enforcementReason: "beta isolation probe",
    },
  });
  expectStatus(
    "tenant-local ban enforced",
    (await request("/api/me", memberB)).status,
    403,
  );
  expectStatus(
    "tenant-local ban does not leak",
    (await request("/api/me", memberA)).status,
    200,
  );

  console.log(
    JSON.stringify({ ok: true, checks: checks.length, stamp, baseUrl }),
  );
} finally {
  await cleanup();
