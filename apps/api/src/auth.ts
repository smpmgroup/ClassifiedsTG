...
export function validateInitData(raw:string, botToken:string, maxAgeSeconds:number, nowSeconds=Math.floor(Date.now()/1000)): { user:TelegramUser; hash:string; queryId?:string; startParam?:string } {
  ...
  return { user, hash:received, queryId:params.get('query_id') || undefined, startParam:params.get('start_param') || undefined };
}

export async function telegramMembership(config:Config, chatId:bigint, userId:number): Promise<string> {
  const response=await fetch(`https://api.telegram.org/bot${config.TELEGRAM_BOT_TOKEN}/getChatMember`, { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({chat_id:chatId.toString(),user_id:userId}) });
